...
void ApplicHoraireWindow::on_pushButtonPlanifier_clicked()
{
    ...
    try {
        ...
        for (int groupIndex : groupIndexes) {
            course.addGroupId(Timetable::getInstance().findGroupByIndex(groupIndex).getId());
        }
        ...
        string courseTuple = Timetable::getInstance().tuple(course);
        addTupleTableCourses(courseTuple);
...
void ApplicHoraireWindow::on_actionOuvrir_triggered()
{
    ...
    const auto& coursesAfter = Timetable::getInstance().getCourses();
    for (const auto& course : coursesAfter) {
...
void ApplicHoraireWindow::on_actionImporterProfesseurs_triggered()
{
    ...
    if (fileName.empty()) {
        dialogError("Erreur", "Aucun fichiers sélectionné !");
        return;
    }
...
void ApplicHoraireWindow::on_actionImporterGroupes_triggered()
{
    ...
    try {
        ...
        // Réactualiser l'affichage dans la table
        clearTableGroups();
        const auto& groups = Timetable::getInstance().getGroups();
        for (const auto& group : groups) {
            addTupleTableGroups(group.tuple());
